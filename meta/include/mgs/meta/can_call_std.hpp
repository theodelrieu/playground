...
#define CAN_CALL_STD_FUNC_IMPL(std_name)                                       \
  ...
  using result_of_##std_name##_t =                                             \
      typename detail2::result_of_##std_name<T...>::type;                      \
                                                                               \
  template <typename... T>                                                     \
  struct can_call_##std_name : detail2::can_call_##std_name##_impl<T...>::type \
  {                                                                            \
  };                                                                           \
                                                                               \
  namespace detail3                                                            \
  ...
  struct would_call_std_##std_name                                             \
    : detail3::would_call_std_##std_name##_impl<T...>::type                    \
  {                                                                            \
  };
